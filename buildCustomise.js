...
function panesMap (panes) {
  return _.map(panes, function (pane) {
    ...
    pane.options = _.filter(pane.options, function (option) {
...
function optionsMap (options) {
  return _.map(options, function (item) {
    ...
    if (_item_ && !_.isUndefined(item.title)) {
      _item_.title = item.title
    }
    if (_item_ && !_.isUndefined(item.defaults)) {
      _item_.defaults = item.defaults
    }
    if (_item_ && !_.isUndefined(item.options)) {
      _item_.options = optionsMap(item.options)
    }
    return _item_
  })
}
